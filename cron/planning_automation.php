...
foreach ($toStart as $intervention) {
    try {
        ...
        // Ne jamais rouvrir un ticket résolu / clôturé (verrouillé)
        $linkedTicket = $db->fetch(
            "SELECT status FROM tickets WHERE id = ?",
            [$intervention['ticket_id']]
        );
        if ($linkedTicket && in_array($linkedTicket['status'], ['resolu', 'cloture'], true)) {
            logMessage("Intervention #{$intervention['id']} ignorée : ticket verrouillé ({$linkedTicket['status']})");
            continue;
        }
        ...
        $notificationModel->createNotification(
            ...
            "▶Intervention {$ticketNumber} démarrée", // SANS #
            $link,
            'planning'
        );
        ...
        try {
            // Email au technicien
            $subject = "▶Intervention {$ticketNumber} démarrée";
            $message = $mailer->getTemplate($subject, "
                <div style='padding:20px;'>
                    <h2 style='color:#2563eb;'>▶Intervention démarrée</h2>
...
foreach ($toCleanup as $intervention) {
    try {
        ...
        $notificationModel->createNotification(
            $intervention['technician_id'],
            "Intervention {$ticketNumber} annulée (trop ancienne)", // SANS #
            "index.php?page=planning",
            'planning'
        );
        
        $cleanedCount++;
        logMessage("Intervention {$ticketNumber} annulée automatiquement");
