    ...
    <div class="w-full px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between items-center h-16">
            ...
            <div class="flex items-center gap-3">
                ...
                <!-- ✅ LOGO SPIDER (remplace l'ancienne icône ticket) -->
                <img src="assets/images/spider-logo-clean.png" alt="SPIDER"
                     class="w-10 h-10 rounded-xl object-contain bg-white shadow-md group-hover:shadow-lg transition-all group-hover:scale-105">
        <span class="logo-text text-xl">Plateforme de Ticket</span>
            ...
            <div class="hidden md:flex items-center gap-1">
                ...
                <?php if (in_array($role, ['admin', 'coordinateur', 'responsable_support_technique', 'responsable_sav', 'responsable_travaux', 'charge_etude_courant_fort', 'charge_etude_courant_faible', 'charge_etude_climatisation', 'commercial'])): ?>
            ...
            <div class="flex items-center gap-2 sm:gap-3">
                ...
                <div class="relative" x-data="{ open: false }">
                    <button @click="open = !open; if(open) loadNotifications()" 
                            ...
                            title="Notifications"
                            id="notificationBell">
                        <i class="fas fa-bell text-xl text-gray-600 transition hover:text-green-600"></i>
                        <span id="liveStatusDot" class="absolute bottom-0.5 right-0.5 w-2 h-2 rounded-full bg-gray-300 transition-colors" title="Connexion temps réel"></span>
                    ...
                    <div x-show="open" @click.away="open = false" 
                        ...
                        <div class="p-4 border-b border-gray-200 flex justify-between items-center sticky top-0 bg-white z-10">
                            <span class="font-semibold text-gray-900 flex items-center gap-2">
                                ...
                                Notifications
                                <span id="notifBadgeCount" class="text-xs bg-green-100 text-green-600 px-2 py-0.5 rounded-full <?= $unreadCount > 0 ? '' : 'hidden' ?>">
                                    <?= $unreadCount ?>
                                </span>
                            </span>
                            <div class="flex items-center gap-2">
                                <!-- Activer/désactiver le son des notifications temps réel -->
                                <button onclick="toggleNotifSound()"
                                        class="text-xs text-gray-500 hover:text-green-700 transition flex items-center"
                                        title="Activer/désactiver le son des notifications">
                                    <i class="fas fa-volume-up"></i>
                                </button>
                                <span class="text-gray-300">|</span>
                                <!-- Tout marquer comme lu - Disponible pour TOUS -->
                                <button onclick="markAllRead()" 
                                        class="text-xs text-green-600 hover:text-green-800 font-medium transition flex items-center gap-1 <?= $unreadCount > 0 ? '' : 'hidden' ?>" 
                                        id="markAllReadBtn">
                        ...
                        <div class="p-2 border-t border-gray-100 text-center bg-gray-50">
                            <a href="index.php?page=historique" class="text-xs text-green-600 hover:text-green-800 font-medium transition flex items-center justify-center gap-1 group">
...
function loadNotifications() {
    ...
    .then(data => {
        ...
        if (data.code === 'AUTH_REQUIRED' || data.error === 'Non authentifié') {
            console.warn('Session API expirée');
            ...
            list.innerHTML = `
                <div class="p-6 text-center text-red-400 text-sm">
                    ...
                    <button onclick="loadNotifications()" 
                            class="mt-2 block text-green-600 hover:text-green-800 text-xs font-medium">
        ...
        if (data.notifications.length > 0) {
            list.innerHTML = data.notifications.map(n => {
                ...
                return `
                    <a href="${link}" 
                       class="block px-4 py-3 hover:bg-gray-50 transition group ${isRead ? '' : 'bg-green-50/50 border-l-4 border-green-500'}">
                        <div class="flex items-start gap-3">
                            <div class="flex-shrink-0 mt-0.5">
                                <div class="w-8 h-8 rounded-full ${isRead ? 'bg-gray-100' : 'bg-green-100'} flex items-center justify-center ${color}">
                            ...
                            <div class="flex-1 min-w-0">
                                <div class="flex items-center justify-between">
                                    ...
                                    ${!isRead ? '<span class="w-2 h-2 bg-green-500 rounded-full flex-shrink-0"></span>' : ''}
                                </div>
...
function updateNotificationBadge(count) {
    ...
    if (count > 0) {
        ...
        if (bellIcon) {
            bellIcon.classList.add('text-green-600');
        ...
        if (bellIcon) {
            bellIcon.classList.remove('text-green-600');
...
// ============================================
// TEMPS RÉEL (SSE) - notifications instantanées
// ============================================
...
const notifSound = new Audio('data:audio/wav;base64,UklGRl9vT19XQVZFZm10IBAAAAABAAEAgD4AAIA+AAABAAgAZGF0YQAAAAA=');

function setLiveDot(status) {
    const dot = document.getElementById('liveStatusDot');
    if (!dot) return;
    dot.classList.remove('bg-gray-300', 'bg-green-500', 'bg-red-400');
    dot.classList.add(status === 'live' ? 'bg-green-500' : status === 'error' ? 'bg-red-400' : 'bg-gray-300');
}

function iconForType(type) {
    const map = {
        ticket: 'fa-ticket-alt', comment: 'fa-comment-dots', status: 'fa-sync-alt',
        action: 'fa-bolt', message: 'fa-envelope', validation: 'fa-check-circle',
        assignation: 'fa-user-check', planning: 'fa-calendar-alt', general: 'fa-bell'
    };
...
function showRealtimeToast(notif) {
    ...
    el.className = `notification-toast notif-${type}`;
    el.innerHTML = `
        <div class="notif-header">
            <div class="notif-icon ${type}"><i class="fas ${iconForType(type)}"></i></div>
            <div class="notif-content">
                <div class="notif-title">${escapeHtml(notif.title || 'Nouvelle notification')}</div>
...
function startLiveNotifications() {
    ...
    try {
        liveEventSource = new EventSource('sse.php?last_id=' + lastNotifId);

        liveEventSource.addEventListener('notification', (e) => {
            setLiveDot('live');
            try {
                const notif = JSON.parse(e.data);
                lastNotifId = notif.id || lastNotifId;
                showRealtimeToast(notif);
                fetch('index.php?page=api&action=count_unread', { credentials: 'include' })
                    .then(r => r.json())
                    .then(d => { if (d && d.count !== undefined) updateNotificationBadge(d.count); })
                    .catch(() => {});
            } catch (err) { console.error('Notif SSE parse error', err); }
        });
        ...
        liveEventSource.onerror = () => {
            setLiveDot('off');
            // Le navigateur retente automatiquement la connexion (cycle serveur ~55s).
            // On ne bascule sur le polling classique qu'après plusieurs échecs consécutifs.
            sseFailCount++;
            if (sseFailCount >= 3) {
...
function toggleNotifSound() {
    ...
    showToast(notifSoundEnabled ? 'Son des notifications activé' : 'Son des notifications désactivé', 'info');
}

// ============================================
// POLLING (repli si le temps réel est indisponible)
// ============================================
    ...
    notificationPolling = setInterval(() => {
        ...
        .then(data => {
            ...
            if (data.error === 'Non authentifié') {
                console.warn('Session API expirée, arrêt du polling');
                stopNotificationPolling();
                return;
            }
        ...
        .catch(err => {
            ...
            if (pollingRetryCount >= MAX_POLLING_RETRY) {
                console.warn('Polling arrêté après plusieurs échecs');
                stopNotificationPolling();
            }
        });
    }, 15000);
