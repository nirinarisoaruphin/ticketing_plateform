...
class Mailer {
    ...
    public function __construct() {
        ...
        if (!$this->enabled) {
            error_log("MAIL_DISABLED: Les emails sont désactivés");
            return;
        }
        ...
        try {
            ...
            // DÉBOGAGE SMTP - piloté par MAIL_DEBUG (config/mail.php)
            // DEBUG_SERVER écrit tout le dialogue SMTP dans error_log :
            // coûteux en E/S, à n'activer que pour un diagnostic.
            ...
            // RÉUTILISATION DE LA CONNEXION SMTP
            // Sans cela, PHPMailer ouvre une nouvelle connexion
            // (TCP + TLS + AUTH, 1 à 3 s) pour CHAQUE destinataire.
            // Avec KeepAlive, une seule connexion sert tout le lot.
            ...
            $this->mail->Timeout = defined('MAIL_TIMEOUT') ? MAIL_TIMEOUT : 15;
    ...
    /**
     * Fermer la connexion SMTP maintenue ouverte par SMTPKeepAlive.
     * À appeler quand un lot d'envois est terminé (cron notamment).
     */
    public function closeConnection() {
        if ($this->enabled && $this->mail) {
            try {
                $this->mail->smtpClose();
            } catch (Exception $e) {
