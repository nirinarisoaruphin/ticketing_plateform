...
class EmailManager {
    ...
    public function send($to, $subject, $message, $toName = '') {
        if (!$this->enabled) {
            $this->log("Envoi bloqué - MAIL_DISABLED: $to");
            return false;
        }
    ...
    /**
     * ENVOI D'UN LOT
     * Si la file est active, on insère en base et on rend la main
     * immédiatement (~1 ms). Sinon on envoie en direct (ancien
     * comportement, lent : 1 à 3 s par destinataire).
     */
    private function dispatch($recipients, $subject, $message, $ticket = null) {
        if (!$this->enabled) {
            $this->log("Envoi bloqué - MAIL_DISABLED");
            return 0;
        }
        
        if (EmailQueue::isEnabled()) {
            $queued = EmailQueue::pushBatch($recipients, $subject, $message, $ticket);
            $this->log("$queued email(s) mis en file d'attente");
            return $queued;
        }
        ...
        if ($this->mailer) {
            $this->mailer->closeConnection();
        }
        $this->log("$count email(s) envoyés en direct");
        return $count;
    }
    ...
    public function notifyTicketCreated($ticket) {
        ...
        if (!$fullTicket) {
            $this->log("Ticket " . $ticket['id'] . " non trouvé en base");
        }
        ...
        $count = $this->dispatch($recipients, $subject, $message, $ticket);
    ...
    public function notifyStatusChange($ticket, $oldStatus, $newStatus) {
        ...
        if (!$fullTicket) {
            $this->log("Ticket " . $ticket['id'] . " non trouvé");
            return false;
        }
     ...
     */
    /**
     * DESTINATAIRES D'UN TICKET
     * Une seule requête au lieu de cinq : les rôles concernés et les
     * deux utilisateurs liés (créateur, assigné) sont récupérés
     * ensemble, puis dédoublonnés par email en PHP.
     */
    private function getEmailRecipients($ticket) {
        ...
        $ids = [];
        if (!empty($ticket['created_by']))  { $ids[] = (int)$ticket['created_by']; }
        if (!empty($ticket['assigned_to'])) { $ids[] = (int)$ticket['assigned_to']; }

        $rolePlaceholders = implode(',', array_fill(0, count($roles), '?'));
        $sql    = "SELECT id, email, full_name, role FROM users WHERE role IN ($rolePlaceholders)";
        $params = $roles;
        ...
        // Libellé affiché : on garde la priorité créateur > assigné > rôle
        $labels = [
            'admin'         => 'Admin',
        ...
        foreach ($rows as $row) {
            $email = $row['email'] ?? '';
            if (empty($email) || isset($recipients[$email])) {
                continue;
            }

            if (!empty($ticket['created_by']) && $row['id'] == $ticket['created_by']) {
                $qualifier = 'Créateur';
            } elseif (!empty($ticket['assigned_to']) && $row['id'] == $ticket['assigned_to']) {
                $qualifier = 'Assigné';
            } elseif ($responsibleRole && $row['role'] === $responsibleRole) {
                $qualifier = 'Responsable';
            } else {
                $qualifier = $labels[$row['role']] ?? 'Destinataire';
            }

            $recipients[$email] = $row['full_name'] . ' (' . $qualifier . ')';
        }

        // Fallback pour les tests
        if (empty($recipients) || (isset($ticket['id']) && $ticket['id'] == 999)) {
    ...
    private function buildActionEmailTemplate($ticket, $actionType, $content, $senderName) {
        ...
        $actionLabels = [
            ...
            'signaler_probleme' => ['label' => 'Problème signalé', 'color' => '#ef4444', 'icon' => 'fa-exclamation-triangle'],
            ...
            'escalader' => ['label' => 'Ticket escaladé', 'color' => '#8b5cf6', 'icon' => 'fa-arrow-up']
        ];
