...
function canManagePlanning() {
    ...
    // Admin : consultation uniquement (comme commercial), pas de gestion
    $allowedRoles = [
        'coordinateur',
        'responsable_support_technique',
        'responsable_sav',
...
function canDeleteTicket($ticket = null) {
    ...
    // Admin : consultation uniquement, pas de suppression de tickets
    if ($role === 'admin') {
        return false;
    }
...
// Un ticket résolu (ou clôturé) est figé :
// plus aucune action ni changement de statut,
// quel que soit le rôle (admin inclus).
// ============================================

/**
 * Statuts qui figent définitivement un ticket
 */
function getLockedTicketStatuses() {
    return ['resolu', 'cloture'];
}

/**
 * Vérifier si un ticket est verrouillé
 * Accepte un tableau ticket ou directement une chaîne de statut.
 */
function isTicketLocked($ticket) {
    ...
    return in_array($status, getLockedTicketStatuses(), true);
}

/**
 * Message expliquant pourquoi le ticket est verrouillé
 */
function getTicketLockMessage($ticket) {
    if (is_array($ticket)) {
        $status = $ticket['status'] ?? '';
    } else {
        $status = (string)$ticket;
    ...
    if ($status === 'resolu') {
        return "Ce ticket est résolu. Il est verrouillé : aucune action ni changement de statut n'est plus possible.";
    }
    if ($status === 'cloture') {
        return "Ce ticket est clôturé. Il est verrouillé : aucune action ni changement de statut n'est plus possible.";
    }
    return '';
}
...
function getRoleLabel($role) {
    $labels = [
        ...
        'charge_etude_courant_fort' => 'Chargé d\'Étude Courant Fort',
...
function getRoleIcon($role) {
    $icons = [
        ...
        'charge_etude_courant_fort' => 'fa-bolt',
...
function getRoleAvatarClass($role) {
    $classes = [
        'admin' => 'bg-red-500',
        'coordinateur' => 'bg-teal-500',
        'responsable_support_technique' => 'bg-green-500',
        'responsable_sav' => 'bg-pink-500',
        'responsable_travaux' => 'bg-amber-500',
        'commercial' => 'bg-blue-500',
        'charge_etude_courant_fort' => 'bg-orange-500',
...
function getRoleColor($role) {
    $colors = [
        'admin' => '#EF4444',
        'coordinateur' => '#8B5CF6',
        'responsable_support_technique' => '#16A34A',
        'responsable_sav' => '#EC4899',
        'responsable_travaux' => '#F59E0B',
        'commercial' => '#3B82F6',
        'charge_etude_courant_fort' => '#F97316',
...
function getRoleBadgeClass($role) {
    $classes = [
        ...
        'charge_etude_courant_fort' => 'badge-charge_etude_courant_fort',
...
function getUserTeam($role) {
    $teams = [
        ...
        'charge_etude_courant_fort' => 'Support Technique',
...
function ensureTicketSequenceSchema(PDO $pdo) {
    ...
    // PERFORMANCE : sans cache persistant, chaque création de ticket
    // exécutait un CREATE TABLE IF NOT EXISTS puis un SHOW INDEX sur la
    // table tickets. Ces deux requêtes de métadonnées ne servent qu'une
    // fois : un marqueur sur disque évite de les rejouer à chaque requête.
    // Supprimez ce fichier pour forcer une nouvelle vérification.
    $marker = __DIR__ . '/../tmp/.ticket_schema_ok';
    if (is_file($marker)) {
        $checked = true;
        return;
    }
    ...
    try {
        ...
        error_log("Impossible d'ajouter la contrainte UNIQUE sur ticket_number : " . $e->getMessage()
            . " — exécutez sql/nettoyage_doublons.sql puis réessayez.");
    }

    $checked = true;

    // Écrire le marqueur pour les requêtes suivantes
...
function getStatusBadgeClass($status) {
    $classes = [
        ...
        'assigne' => 'bg-teal-100 text-teal-800 border-teal-200',
...
function getNavLinks($role) {
    ...
    $planningRoles = ['admin', 'coordinateur', 'responsable_support_technique', 'responsable_sav', 'responsable_travaux', 'charge_etude_courant_fort', 'charge_etude_courant_faible', 'charge_etude_climatisation', 'commercial'];
