...
class NotificationManager {
    ...
    public function notifyAction($ticket, $actionType, $content, $senderName) {
        ...
        if ($commercial && !empty($commercial['phone'])) {
            ...
            $this->log("Commercial sans numéro de téléphone pour le ticket {$ticket['ticket_number']}");
        }
    ...
    private function sendWhatsAppAssignment($ticket, $assignedTo, $assignedBy, $commercial) {
        ...
        $message .= "*Assigné à :* " . $assignedName . "\n";
        $message .= "*Par :* " . $assignedBy . "\n";
        $message .= "*Titre :* " . $ticketTitle . "\n";
        $message .= "*Date :* " . date('d/m/Y à H:i') . "\n\n";
        $message .= "*Lien :* " . $ticketUrl . "\n\n";
        $message .= "---\n";
    ...
    private function sendWhatsAppAssignmentToTech($ticket, $assignedUser, $assignedBy) {
        ...
        $message .= "*Assigné par :* " . $assignedBy . "\n";
        $message .= "*Titre :* " . $ticketTitle . "\n";
    ...
    private function createInAppNotification($ticket, $actionType, $content, $senderName) {
        ...
        $actionLabels = [
            ...
            'signaler_probleme' => "Problème signalé sur $ticketNumber par $senderName",
    ...
    /**
     * ENVOI D'UN LOT
     * File d'attente si activée (réponse immédiate), sinon envoi direct.
     */
    private function dispatchEmails($recipients, $subject, $message, $ticket = null) {
        if (empty($recipients)) {
            return 0;
        }
    ...
    private function sendSingleEmail($ticket, $actionType, $content, $senderName) {
        ...
        if (empty($recipients)) {
            $this->log("Aucun destinataire pour l'action");
            return;
        }
        ...
        $count = $this->dispatchEmails($recipients, $subject, $message, $ticket);
    ...
    private function sendCommentEmail($ticket, $comment, $author) {
        ...
        if (empty($recipients)) {
            $this->log("Aucun destinataire pour le commentaire");
            return;
        }
    ...
    private function sendStatusChangeEmail($ticket, $oldStatus, $newStatus, $senderName) {
        ...
        if (empty($recipients)) {
            $this->log("Aucun destinataire pour le changement de statut");
            return;
        }
    ...
    private function sendAssignmentEmail($ticket, $assignedTo, $assignedBy) {
        ...
        if (empty($recipients)) {
            $this->log("Aucun destinataire pour l'assignation");
            return;
        }
    ...
    private function buildEmailTemplate($ticket, $actionType, $content, $senderName) {
        ...
        $actionLabels = [
            ...
            'signaler_probleme' => ['label' => 'Problème signalé', 'color' => '#ef4444'],
