...
<div class="planning-container w-full px-4 sm:px-6 lg:px-8 py-8">
    ...
    <div class="filter-card mb-6">
        <form method="GET" class="flex flex-wrap gap-4 items-end" id="filterForm">
            ...
            <div>
                ...
                <select name="technician" class="mt-1 px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-green-500 focus:border-transparent">
            ...
            <div>
                ...
                <select name="status" class="mt-1 px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-green-500 focus:border-transparent">
                    <option value="">Tous</option>
            ...
            <div>
                ...
                <input type="date" name="date_from" id="date_from" value="<?= $_GET['date_from'] ?? '' ?>" 
                       class="mt-1 px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-green-500 focus:border-transparent">
            </div>
            <div>
            ...
            <div class="flex gap-2">
                <button type="submit" class="bg-green-600 hover:bg-green-700 text-white px-4 py-2 rounded-lg text-sm transition font-medium">
                    Filtrer
                </button>
        ...
        <?php else: ?>
                ...
                <div class="intervention-card status-<?= $statusClass ?> bg-white rounded-xl shadow-sm border border-gray-200 p-4 hover:shadow-md transition">
                    ...
                    <div class="mb-2">
                        <a href="index.php?page=tickets&action=show&id=<?= $intervention['ticket_id'] ?>" 
                           class="text-green-600 hover:text-green-800 font-medium text-sm hover:underline">
                    ...
                    <!-- ACTIONS -->
                    <?php $ticketLocked = isTicketLocked($intervention['ticket_status'] ?? ''); ?>
                    <?php if ($canManage && $ticketLocked): ?>
                    <!-- Ticket résolu / clôturé : intervention figée -->
                    <div class="mt-3 pt-3 border-t border-gray-100 text-center">
                        <span class="text-xs text-gray-400 flex items-center justify-center gap-1"
                              title="<?= htmlspecialchars(getTicketLockMessage($intervention['ticket_status'] ?? '')) ?>">
                            <i class="fas fa-lock"></i> Ticket verrouillé &mdash; aucune action possible
                        </span>
                    </div>
                    ...
                    <div class="mt-3 pt-3 border-t border-gray-100 flex gap-2 flex-wrap">
                        <a href="index.php?page=planning&action=edit&id=<?= $intervention['id'] ?>" 
                           class="text-green-600 hover:text-green-800 text-xs font-medium transition p-1 rounded hover:bg-green-50">
                            Modifier
                        </a>
                        ...
                        <a href="index.php?page=planning&action=cancel&id=<?= $intervention['id'] ?>" 
                           ...
                           class="btn-annuler-link text-xs font-medium transition p-1 rounded ml-auto">
                            Annuler
                        </a>
                        <?php endif; ?>
                    </div>
            ...
            <div class="mt-4 px-4 py-3 bg-gray-50 rounded-xl border border-gray-200 flex justify-between items-center">
                ...
                <span class="text-xs text-gray-400 flex gap-3">
                    ...
                    <span>▶Démarrer</span>
                    <span>Supprimer</span>
                    <span class="text-red-600">Annuler</span>
                </span>
                <?php else: ?>
...
function checkPlanningStatus() {
    ...
    .then(data => {
        if (data.updated) {
            // Rechargement automatique supprimé : on informe simplement
            // l'utilisateur via un toast, sans recharger la page (cela
            // évitait de perdre une action en cours - formulaire ouvert,
            // filtre appliqué, etc.). Un bouton permet de rafraîchir
...
function showPlanningToast(message) {
    ...
    toast.className = 'planning-toast fixed top-4 right-4 z-50 bg-green-600 text-white px-5 py-3.5 rounded-xl shadow-xl flex items-center gap-3 max-w-sm animate-slide-in';
    toast.innerHTML = `
        <div class="flex items-center gap-3">
            <i class="fas fa-robot text-white text-lg"></i>
            <div>
                <p class="font-medium text-sm">Automatisation</p>
                <p class="text-xs text-green-100">${message}</p>
            </div>
            <button onclick="location.reload()" class="ml-2 text-xs bg-white/20 hover:bg-white/30 px-2 py-1 rounded-md whitespace-nowrap">
                Actualiser
            </button>
            <button onclick="this.parentElement.parentElement.remove()" class="ml-1 text-white/70 hover:text-white">
