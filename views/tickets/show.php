...
<?php
// URL directe vers ce ticket, encodée dans le QR code
$qrTicketUrl = (defined('APP_URL') ? APP_URL : '') . '/index.php?page=tickets&action=show&id=' . $ticket['id'];
    ...
    <div class="bg-white rounded-xl shadow-xl max-w-sm w-full p-6 text-center">
        <div class="flex justify-between items-center mb-4">
            <h3 class="font-semibold text-gray-900">
                <i class="fas fa-qrcode mr-1"></i> QR Code du ticket
            </h3>
        ...
        <img src="<?= htmlspecialchars($qrImageUrl) ?>" alt="QR Code ticket <?= htmlspecialchars($ticket['ticket_number']) ?>"
             class="mx-auto rounded-lg border border-gray-200" width="260" height="260" id="qrCodeImg">
        <p class="text-sm text-gray-500 mt-3"><?= htmlspecialchars($ticket['ticket_number']) ?></p>
        <p class="text-xs text-gray-400 mt-1">Scannez pour ouvrir directement ce ticket</p>
        <div class="flex gap-2 mt-4">
            <a href="<?= htmlspecialchars($qrImageUrl) ?>" download="qrcode_<?= htmlspecialchars($ticket['ticket_number']) ?>.png"
               class="btn btn-outline btn-sm flex-1">
                <i class="fas fa-download"></i> Télécharger
            </a>
            <button type="button" onclick="window.print()" class="btn btn-outline btn-sm flex-1">
