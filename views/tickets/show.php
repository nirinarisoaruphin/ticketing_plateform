...
// ============================================
// VERROU : ticket résolu / clôturé
// Les boutons restent visibles mais sont grisés.
// ============================================
$ticketLocked    = $ticketLocked    ?? (function_exists('isTicketLocked') ? isTicketLocked($ticket) : false);
$lockMessage     = $lockMessage     ?? (function_exists('getTicketLockMessage') ? getTicketLockMessage($ticket) : '');
$canEditByRole   = $canEditByRole   ?? $canEdit;
$canDeleteByRole = $canDeleteByRole ?? false;
$canActByRole    = $canActByRole    ?? $canAct;
...
<div class="page-wrapper">
    <div class="content max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8 page-enter">
            ...
            <div class="px-6 py-4 bg-gradient-to-r from-blue-50 to-green-50 border-b border-gray-200">
                <div class="flex flex-wrap justify-between items-center gap-3">
                    ...
                    <div class="flex flex-wrap gap-2">
                        <?php if ($ticketLocked && $canEditByRole && $role !== 'commercial'): ?>
                        <span class="btn btn-primary btn-sm is-locked" title="<?= htmlspecialchars($lockMessage) ?>">
                            <i class="fas fa-lock"></i> Modifier
                        </span>
                        <?php elseif ($canEdit && $role !== 'commercial'): ?>
                        ...
                        <?php if ($ticketLocked && function_exists('canDeleteTicket') && canDeleteTicket($ticket)): ?>
                        <span class="btn btn-danger btn-sm is-locked" title="<?= htmlspecialchars($lockMessage) ?>">
                            <i class="fas fa-lock"></i> Supprimer
                        </span>
                        <?php elseif (!$ticketLocked && function_exists('canDeleteTicket') && canDeleteTicket($ticket)): ?>
            ...
            <div class="px-6 py-4">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                    ...
                    <div class="space-y-3">
                        ...
                        <?php if ($canAssign && !empty($chargesEtude)): ?>
                        <div class="flex items-start gap-2">
                            <span class="text-sm text-gray-500 w-32 flex-shrink-0 mt-1">Assigner à</span>
                            <div class="flex flex-wrap gap-2">
                                <?php foreach ($chargesEtude as $charge): ?>
                                <form method="POST" action="index.php?page=tickets&action=quick_assign" class="inline">
                                    <input type="hidden" name="id" value="<?= $ticket['id'] ?>">
                                    <input type="hidden" name="assigned_to" value="<?= $charge['id'] ?>">
                                    <button type="submit"
                                        onclick="return confirm('Assigner ce ticket à <?= htmlspecialchars($charge['full_name'], ENT_QUOTES) ?> ?');"
                                        class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium border transition
                                        <?= (!empty($ticket['assigned_to']) && $ticket['assigned_to'] == $charge['id'])
                                            ? 'bg-green-600 text-white border-green-600'
                                            : 'bg-white text-gray-700 border-gray-300 hover:bg-green-50 hover:border-green-400 hover:text-green-700' ?>">
                                        <i class="fas fa-share"></i>
                                        <?= htmlspecialchars($charge['full_name']) ?>
                                        <?php if (!empty($ticket['assigned_to']) && $ticket['assigned_to'] == $charge['id']): ?>
        ...
        <!-- ============================================ -->
        <?php if ($ticketLocked && $canActByRole && $role !== 'commercial'): ?>
        <!-- Ticket verrouillé : actions affichées mais désactivées -->
        <div class="mt-6 bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden actions-locked">
            <div class="px-6 py-4 border-b border-gray-200 bg-gray-50/50 flex items-center justify-between gap-3 flex-wrap">
                <h3 class="text-sm font-semibold text-gray-700">
                    <i class="fas fa-tools text-gray-400 mr-2"></i>
                    Actions sur le ticket
                </h3>
                <span class="lock-pill"><i class="fas fa-lock mr-1"></i> Verrouillé</span>
            </div>
            <div class="px-6 py-4">
                <div class="grid grid-cols-2 md:grid-cols-4 gap-2 locked-zone">
                    <button type="button" disabled class="px-4 py-2.5 text-sm font-medium rounded-lg bg-teal-100 text-teal-700 border-2 border-transparent">
                        <i class="fas fa-spinner mr-1"></i> En cours
                    </button>
                    <button type="button" disabled class="px-4 py-2.5 text-sm font-medium rounded-lg bg-green-100 text-green-700 border-2 border-transparent">
                        <i class="fas fa-check-circle mr-1"></i> Résolu
                    </button>
                    <button type="button" disabled class="px-4 py-2.5 text-sm font-medium rounded-lg bg-yellow-100 text-yellow-700 border-2 border-transparent">
                        <i class="fas fa-clock mr-1"></i> En attente
                    </button>
                    <button type="button" disabled class="px-4 py-2.5 text-sm font-medium rounded-lg bg-red-100 text-red-700 border-2 border-transparent">
                        <i class="fas fa-exclamation-triangle mr-1"></i> Signaler
                    </button>
                </div>
                <div class="mt-4 locked-zone">
                    <label class="block text-sm font-medium text-gray-700">Message</label>
                    <textarea rows="2" disabled
                              class="mt-1 w-full px-4 py-2.5 border border-gray-300 rounded-lg bg-gray-50"
                             ></textarea>
                </div>
                <button type="button" disabled class="mt-4 w-full py-2.5 bg-blue-600 text-white font-semibold rounded-lg locked-zone">
                    <i class="fas fa-lock mr-2"></i> Action impossible
                </button>
            </div>
        </div>
        <?php elseif ($canAct && $role !== 'commercial'): ?>
                ...
                <form method="POST" action="index.php?page=tickets&action=action" class="space-y-4">
                    ...
                    <div class="grid grid-cols-2 md:grid-cols-4 gap-2">
                        <button type="submit" name="action_type" value="en_cours"
                                class="px-4 py-2.5 text-sm font-medium rounded-lg transition bg-teal-100 text-teal-700 hover:bg-teal-200 border-2 border-transparent hover:border-teal-300">
                            <i class="fas fa-spinner mr-1"></i> En cours
                        </button>
                    ...
                    <div>
                        <label class="block text-sm font-medium text-gray-700">Message (Signaler + Message)</label>
            ...
            <div class="px-6 py-4 max-h-64 overflow-y-auto">
                ...
                <div class="space-y-2">
                    <?php foreach ($actions as $action): 
                        $actionLabels = [
                            'signaler_probleme' => ['label' => 'Problème signalé', 'class' => 'text-red-600 bg-red-50'],
                            ...
                            'escalader' => ['label' => 'Escaladé', 'class' => 'text-orange-600 bg-orange-50'],
                            'resolu' => ['label' => 'Résolu', 'class' => 'text-green-600 bg-green-50'],
                            'en_cours' => ['label' => 'En cours', 'class' => 'text-teal-600 bg-teal-50'],
...
function confirmDeleteTicket(ticketNumber) {
    return confirm(
        'Êtes-vous sûr de vouloir supprimer le ticket ' + ticketNumber + ' ?\n\n' +
