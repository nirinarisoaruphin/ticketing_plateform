...
class PlanningController {
    ...
    // ============================================
    // VERROU TICKET
    // Un ticket résolu / clôturé est figé : le planning
    // ne doit plus pouvoir modifier son statut.
    // ============================================
    private function isLinkedTicketLocked($ticketId) {
        if (empty($ticketId)) {
            return false;
        }
        $ticket = $this->db->fetch("SELECT status FROM tickets WHERE id = ?", [(int)$ticketId]);
        return $ticket ? isTicketLocked($ticket) : false;
    }

    private function blockIfTicketLocked($ticketId, $redirectTo = 'index.php?page=planning') {
        if ($this->isLinkedTicketLocked($ticketId)) {
            $ticket = $this->db->fetch("SELECT status FROM tickets WHERE id = ?", [(int)$ticketId]);
    ...
    public function create() {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            $this->blockIfTicketLocked($_POST['ticket_id'], 'index.php?page=planning&action=create');
    ...
    public function edit() {
        ...
        $this->blockIfTicketLocked($intervention['ticket_id']);
    ...
    public function delete() {
        ...
        // Un ticket résolu / clôturé reste verrouillé : on ne le remet pas à "nouveau"
        if (!$this->isLinkedTicketLocked($intervention['ticket_id'])) {
            $this->ticketModel->update($intervention['ticket_id'], [
                'status' => 'nouveau',
                'assigned_to' => null
            ]);
