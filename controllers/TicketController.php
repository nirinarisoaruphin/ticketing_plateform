...
class TicketController {
    ...
    public function show() {
        ...
        if ($role === 'admin' || $role === 'coordinateur') {
            ...
            $canView = true;
        } elseif (in_array($role, ['charge_etude_courant_fort', 'charge_etude_courant_faible', 'charge_etude_climatisation'])) {
        ...
        // Verrou : ticket résolu / clôturé -> boutons affichés mais grisés
        $ticketLocked = isTicketLocked($ticket);
        $lockMessage = getTicketLockMessage($ticket);
        $canEditByRole = $this->canEditTicketByRole($ticket);
        $canDeleteByRole = $this->canDeleteTicketByRole($ticket);
        $canActByRole = $this->canActOnTicketByRole($ticket);
        ...
        if ($role === 'responsable_support_technique' || in_array($role, ['admin', 'coordinateur'])) {
            $chargesEtude = $db->fetchAll(
                "SELECT id, full_name, role, email FROM users 
                 WHERE role IN ('charge_etude_courant_fort', 'charge_etude_courant_faible', 'charge_etude_climatisation') 
                 ORDER BY full_name"
            );
        }
    ...
    public function create() {
        ...
        $role = $_SESSION['user_role'] ?? 'commercial';
        // Admin : consultation uniquement, pas de création de tickets
        if ($role === 'admin') {
            setFlash('danger', 'Vous n\'avez pas la permission de créer un ticket (consultation uniquement).');
            redirect('index.php?page=tickets');
        }
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            // ============================================
            // PROTECTION SERVEUR ANTI-DOUBLON (filet de sécurité)
            // ============================================
            // En complément du jeton de soumission unique, on vérifie
            // qu'aucun ticket identique (même auteur, même catégorie,
            // même description) n'a été créé dans les 20 dernières
            // secondes. Cela bloque tout doublon même si deux requêtes
            // HTTP arrivent en parallèle (double-clic, double onglet,
            // requête réseau dupliquée, bug JS, etc.), quelle qu'en
            // soit la cause côté navigateur.
            $recentDuplicate = $db->fetch(
                ...
                [$_SESSION['user_id'], $category, sanitize($_POST['description'])]
            );
            ...
            if ($recentDuplicate) {
                if ($this->isAjax()) {
                    $this->jsonResponse([
                        'success' => true,
                        'ticket_id' => $recentDuplicate['id'],
                        'ticket_number' => $recentDuplicate['ticket_number'],
            ...
            if (isset($_FILES['attachment']) && $_FILES['attachment']['error'] === UPLOAD_ERR_OK) {
                ...
                // Sécurité : seules les extensions de fichiers non exécutables sont acceptées.
                ...
                if (in_array($extension, $allowedExtensions, true)) {
                    ...
                    error_log("Upload rejeté : extension non autorisée (" . $extension . ") par l'utilisateur " . ($_SESSION['user_id'] ?? '?'));
                }
            }
                ...
                try {
                    ...
                    if ($commercial && !empty($commercial['phone'])) {
                        ...
                        if ($phone) {
                            ...
                            $whatsappMessage .= "*Titre :* " . $ticketTitle . "\n";
    ...
    public function edit() {
        ...
        if (isTicketLocked($ticket)) {
            setFlash('danger', getTicketLockMessage($ticket));
            redirect('index.php?page=tickets&action=show&id=' . $id);
        }
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if (isTicketLocked($ticket)) {
                if ($this->isAjax()) {
                    $this->jsonResponse(['success' => false, 'error' => getTicketLockMessage($ticket)]);
                }
                setFlash('danger', getTicketLockMessage($ticket));
                redirect('index.php?page=tickets&action=show&id=' . $id);
            }
            ...
            if ($newStatus === 'cloture') {
                ...
                if (in_array($role, array('coordinateur', 'responsable_support_technique', 'responsable_sav', 'responsable_travaux'))) {
    ...
    public function addAction() {
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            // Ticket résolu / clôturé : aucune action, aucun changement de statut
            if (isTicketLocked($ticket)) {
                if ($this->isAjax()) {
                    $this->jsonResponse(['success' => false, 'error' => getTicketLockMessage($ticket)]);
                }
            ...
            if (!empty($content) || in_array($actionType, ['resolu', 'en_cours', 'en_attente'])) {
                ...
                try {
                    ...
                    // PERFORMANCE : on regarde D'ABORD si le commercial a un
                    // numéro de téléphone. Sans numéro, l'image ne serait
                    // envoyée à personne : inutile de la générer (la génération
                    // GD d'un PNG 800x600 coûte plusieurs centaines de ms).
                    $commercial = $db->fetch("SELECT id, full_name, phone FROM users WHERE id = ?", [$ticket['created_by']]);
                    $phone = ($commercial && !empty($commercial['phone'])) ? cleanPhoneNumber($commercial['phone']) : null;
    ...
    public function processTicket() {
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            // Ticket résolu / clôturé : changement de statut interdit
            if (isTicketLocked($ticket)) {
                if ($this->isAjax()) {
                    $this->jsonResponse(['success' => false, 'error' => getTicketLockMessage($ticket)]);
                }
            ...
            // Admin (et autres non autorisés) : pas d'actions sur les tickets
            if (!$this->canActOnTicket($ticket)) {
                if ($this->isAjax()) {
                    $this->jsonResponse(['success' => false, 'error' => 'Permission refusée']);
                }
                setFlash('danger', 'Vous n\'avez pas la permission d\'effectuer cette action.');
                redirect('index.php?page=tickets&action=show&id=' . $id);
            }
    ...
    public function validateTicket() {
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            // Ticket résolu / clôturé : plus de validation possible
            if (isTicketLocked($ticket)) {
                setFlash('danger', getTicketLockMessage($ticket));
                redirect('index.php?page=tickets&action=show&id=' . $id);
            }
            ...
            // Admin : consultation uniquement, pas de validation
            if (!in_array($role, ['coordinateur', 'responsable_support_technique', 'responsable_sav', 'responsable_travaux'])) {
    ...
    private function canViewTicket($ticket) {
        ...
        if (in_array($role, ['charge_etude_courant_fort', 'charge_etude_courant_faible', 'charge_etude_climatisation'])) {
            return in_array($ticketCategory, ['support_technique', 'bureau_etude']);
        }
    ...
    /**
     * Droit de modification lié au rôle uniquement (sans tenir compte du verrou).
     * Sert à afficher les boutons grisés sur un ticket verrouillé.
     */
    private function canEditTicket($ticket) {
        // Ticket résolu ou clôturé : verrouillé pour tout le monde
        if (isTicketLocked($ticket)) {
            return false;
        }
        return $this->canEditTicketByRole($ticket);
    }
    
    private function canEditTicketByRole($ticket) {
        $role = $_SESSION['user_role'] ?? 'commercial';
        $ticketCategory = $ticket['category'] ?? '';
    ...
    private function canDeleteTicket($ticket) {
        ...
        return $this->canDeleteTicketByRole($ticket);
    }
    
    private function canDeleteTicketByRole($ticket) {
        $role = $_SESSION['user_role'] ?? 'commercial';
        $ticketCategory = $ticket['category'] ?? '';
        
        // Admin : consultation uniquement, pas de suppression
        if ($role === 'admin') {
            return false;
        }
    ...
    private function canCommentTicket($ticket) {
        ...
        // Admin : consultation uniquement, pas de commentaire (action)
        if ($role === 'admin') {
            return false;
        }
    ...
    private function canActOnTicket($ticket) {
        // Ticket résolu ou clôturé : verrouillé, plus aucune action
        if (isTicketLocked($ticket)) {
            return false;
        }
        return $this->canActOnTicketByRole($ticket);
    }
    
    /**
     * Droit d'action lié au rôle uniquement (sans tenir compte du verrou).
     */
    private function canActOnTicketByRole($ticket) {
        $role = $_SESSION['user_role'] ?? 'commercial';
        
        if ($role === 'commercial' || $role === 'admin') {
            // Admin et commercial : consultation uniquement, pas d'actions
            return false;
        }
    ...
    // ============================================
    // ASSIGNATION RAPIDE (bouton "Assigner à" sur la fiche ticket)
    // ============================================
    public function quickAssign() {
        ...
        $db = Database::getInstance();
        $target = $db->fetch(
            "SELECT id, full_name, role FROM users WHERE id = ? AND active = 1 AND role IN ('charge_etude_courant_fort', 'charge_etude_courant_faible', 'charge_etude_climatisation')",
            [$assignedTo]
        );

        if (!$target) {
            setFlash('danger', 'Destinataire invalide.');
            redirect($link);
        }

        $data = array('assigned_to' => $target['id']);
        if (in_array($ticket['status'], ['nouveau'])) {
            $data['status'] = 'assigne';
        }
        ...
        try {
            $this->notificationManager->notifyAssignment($updatedTicket, $target['id'], $_SESSION['user_name'] ?? 'Mahery');
        } catch (Exception $e) {
            error_log("Erreur notification d'assignation: " . $e->getMessage());
        }

        setFlash('success', 'Ticket assigné à ' . $target['full_name'] . ' avec succès !');
        redirect($link);
    }
    ...
    private function canAssignTicket($ticket) {
        ...
        if ($role === 'commercial' || $role === 'admin') {
            // Admin et commercial : consultation uniquement, pas d'assignation
            return false;
        }
        ...
        if (!in_array($role, ['responsable_support_technique', 'coordinateur'])) {
            return false;
        }
