...
class Ticket extends Model {
    ...
    public function assignToMultiple($ticketId, $userIds, $assignedBy) {
        ...
        if ($success > 0) {
            $current = $db->fetch("SELECT status FROM tickets WHERE id = ?", [$ticketId]);
            $isLocked = (function_exists('isTicketLocked') && $current) ? isTicketLocked($current) : false;
            
            // Garde-fou : un ticket résolu / clôturé ne repasse pas à "assigné"
            if (!$isLocked) {
                $db->query(
    ...
    public function create($data) {
        ...
        if (!isset($data['assigned_to']) || $data['assigned_to'] === null || $data['assigned_to'] === '') {
            error_log("assigned_to est NULL ou vide dans create()");
        ...
        for ($attempt = 1; $attempt <= $maxAttempts; $attempt++) {
            try {
                ...
                if ($isDuplicateKey && $attempt < $maxAttempts && isset($filteredData['ticket_number'])) {
                    error_log("Collision ticket_number (tentative $attempt), régénération...");
    ...
    public function updateStatus($ticketId, $status, $userId = null) {
        ...
        $ticket = $db->fetch("SELECT id, ticket_number, status FROM tickets WHERE id = ?", [$ticketId]);
        if (!$ticket) {
            return false;
        }
        
        // Garde-fou : un ticket résolu / clôturé est figé
        if (function_exists('isTicketLocked') && isTicketLocked($ticket)) {
            return false;
        }
