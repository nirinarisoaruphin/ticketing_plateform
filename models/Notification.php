...
class Notification {
    ...
    public function notifyTechnicians($message, $link = null, $type = 'general') {
        $technicians = $this->db->fetchAll(
            "SELECT id FROM users WHERE role IN ('responsable_support_technique', 'responsable_sav', 'responsable_travaux', 'charge_etude_courant_fort', 'charge_etude_courant_faible', 'charge_etude_climatisation')"
        );
